...
for roinum=rois
    ...
    roiPrf{iroi}.ang = ang;
    roiPrf{iroi}.sz = sz;
    roiPrf{iroi}.exponent = exponent;
    roiPrf{iroi}.gain = gain;
    roiPrf{iroi}.r2 = r2;
...
%                     voxPrfSampleOri(iori) = prfResp(modelOri{ilev}(iori,:,:),X,Y,x0(ivox),y0(ivox),sz(ivox),exponent(ivox),gain(ivox));
    ...
    prfSampleLev{iroi} = prfSampleLevRoi;
    prfSampleLevOri{iroi} = prfSampleLevOriRoi;
end

% end
prffolder = '~/NSD/prfsample/';
save(fullfile(prffolder,['prfSampleStim_v1_sub' num2str(isub) '.mat']),'prfSampleLevOri','prfSampleLev',...
